...
/////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
///PASSING SMART POINTERS --> C++ core guidelines since they have six rules for passing std::shared_ptr and std::unique_ptr.
...
void sink(std::unique_ptr<Widget> uniqPtr) //! If the function only wants to use the Widget , it should take its parameter by pointer or by reference. A pointer can be a null pointer, but a reference cannot.
{
    //do sth with unique ptr
}
int main()
{
    auto unique_ptr = std::make_unique<Widget>(1998);
    sink(std::move(unique_ptr));
    sink(unique_ptr);            //(2) ERROR because we cannot copy an std::unique_ptr --> khodam: albate benzaram in ke dg move shode ke dg ghabele use nist
}
...
void reseat(std::unique_ptr<Widget>& uniqPtr){
        uniqPtr.reset(new Widget{2003}); //This call will not only construct a new  Widget(2003) , but it will also destruct the old Widget(1998) .
        //do sth with uniq ptr
}
...
int main()
{
    ...
    reseat(std::move(uniqPtr)); //ERROR -->because we cannot bind an rvalue to a non-const lvalue reference
    reseat(uniqPtr);  //in error nemide chun --> An lvalue can be bound to an lvalue reference.
...
! nor can we reseat the resource. To be honest, we should use a pointer    ---> albate const shared_ptr& ptr yani nemitoni khode pointer ro avaz kuni ke be koja point mikune vali mishe resource dakhelesho dobare set kard -->inja bishatar manzor ine ke reset va sepas reseat nemishe kard
! ( Widget* ) or a reference ( Widget& ) as a parameter instead, because there
! is no added value in using an std::shared_ptr . --> Profound reasoning :)
*/



//! PROBLEM ---> moshkele paeen ke zemanat nemishe vojod wid dar oldFun ro chejori hal kunim??
void oldFunc(Widget* wid){ //! Here is the problem-->oldFunc accepts a pointer to the Widget; therefore , oldFunc has no guarantee that the Widget will stay alive during its execution--> oldFunc only borrows the widget.
    // do sth wit wid
}

void shared(std::shared_ptr<Widget>& shaPtr){ // (2) --> it takes its argument by reference--> so the reference counter of shaPtr will not be increased and the function shared will not extend the lifetime of Widget(2011)
...
auto globalShared = std::make_shared<Widget>(2011) // (1) //globShared is a globally shared pointer.
...
//! SOLUTION --->The solution is quite simple. We must ensure that the reference count of globShared is increased before the call to the function oldFunc , meaning that we must make a copy of std::shared_ptr :
